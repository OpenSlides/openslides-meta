...
CREATE TABLE action_worker_t (
    ...
    name varchar(256) CONSTRAINT required_action_worker_name NOT NULL,
    state varchar(256) CONSTRAINT required_action_worker_state NOT NULL CONSTRAINT enum_action_worker_state CHECK (state IN ('running', 'end', 'aborted')),
    created timestamptz CONSTRAINT required_action_worker_created NOT NULL,
    timestamp timestamptz CONSTRAINT required_action_worker_timestamp NOT NULL,
    result jsonb,
    user_id integer CONSTRAINT required_action_worker_user_id NOT NULL
);
...
CREATE TABLE agenda_item_t (
    ...
    comment varchar(256),
    closed boolean CONSTRAINT default_agenda_item_closed DEFAULT False,
    type varchar(256) CONSTRAINT enum_agenda_item_type CHECK (type IN ('common', 'internal', 'hidden')) CONSTRAINT default_agenda_item_type DEFAULT 'common',
    duration integer CONSTRAINT minimum_agenda_item_duration CHECK (duration >= 0),
    is_internal boolean,
    is_hidden boolean,
    level integer,
    weight integer,
    content_object_id varchar(100) CONSTRAINT required_agenda_item_content_object_id NOT NULL,
    content_object_id_motion_id integer CONSTRAINT unique_agenda_item_content_object_id_motion_id UNIQUE CONSTRAINT generated_always_as_agenda_item_content_object_id_motion_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'motion' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_motion_block_id integer CONSTRAINT unique_agenda_item_content_object_id_motion_block_id UNIQUE CONSTRAINT generated_always_as_agenda_item_content_object_id_motion6edbb3a GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'motion_block' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_assignment_id integer CONSTRAINT unique_agenda_item_content_object_id_assignment_id UNIQUE CONSTRAINT generated_always_as_agenda_item_content_object_id_assignment_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'assignment' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_topic_id integer CONSTRAINT unique_agenda_item_content_object_id_topic_id UNIQUE CONSTRAINT generated_always_as_agenda_item_content_object_id_topic_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'topic' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    ...
    parent_id integer,
    meeting_id integer CONSTRAINT required_agenda_item_meeting_id NOT NULL
);
...
CREATE TABLE assignment_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    title varchar(256) CONSTRAINT required_assignment_title NOT NULL,
    description text,
    open_posts integer CONSTRAINT minimum_assignment_open_posts CHECK (open_posts >= 0) CONSTRAINT default_assignment_open_posts DEFAULT 0,
    phase varchar(256) CONSTRAINT enum_assignment_phase CHECK (phase IN ('search', 'voting', 'finished')) CONSTRAINT default_assignment_phase DEFAULT 'search',
    default_poll_description text,
    number_poll_candidates boolean,
    sequential_number integer CONSTRAINT required_assignment_sequential_number NOT NULL,
    CONSTRAINT unique_assignment_sequential_number_meeting_id UNIQUE (sequential_number, meeting_id),
    meeting_id integer CONSTRAINT required_assignment_meeting_id NOT NULL
);
...
CREATE TABLE assignment_candidate_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    weight integer CONSTRAINT default_assignment_candidate_weight DEFAULT 10000,
    assignment_id integer CONSTRAINT required_assignment_candidate_assignment_id NOT NULL,
    meeting_user_id integer,
    meeting_id integer CONSTRAINT required_assignment_candidate_meeting_id NOT NULL
);
...
CREATE TABLE chat_group_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_chat_group_name NOT NULL,
    weight integer CONSTRAINT default_chat_group_weight DEFAULT 10000,
    meeting_id integer CONSTRAINT required_chat_group_meeting_id NOT NULL,
...
CREATE TABLE chat_message_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    content text CONSTRAINT required_chat_message_content NOT NULL,
    created timestamptz CONSTRAINT required_chat_message_created NOT NULL,
    meeting_user_id integer,
    chat_group_id integer CONSTRAINT required_chat_message_chat_group_id NOT NULL,
    meeting_id integer CONSTRAINT required_chat_message_meeting_id NOT NULL
);
...
CREATE TABLE committee_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_committee_name NOT NULL,
    description text,
    ...
    parent_id integer,
    organization_id integer CONSTRAINT required_committee_organization_id NOT NULL CONSTRAINT default_committee_organization_id DEFAULT 1
);
...
CREATE TABLE gender_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_gender_name NOT NULL CONSTRAINT unique_gender_name UNIQUE,
    organization_id integer CONSTRAINT required_gender_organization_id NOT NULL CONSTRAINT default_gender_organization_id DEFAULT 1
);
...
CREATE TABLE group_t (
    ...
    name varchar(256) CONSTRAINT required_group_name NOT NULL,
    ...
    meeting_id integer CONSTRAINT required_group_meeting_id NOT NULL,
...
CREATE TABLE history_entry_t (
    ...
    model_id_user_id integer CONSTRAINT generated_always_as_history_entry_model_id_user_id GENERATED ALWAYS AS (CASE WHEN split_part(model_id, '/', 1) = 'user' THEN cast(split_part(model_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    model_id_motion_id integer CONSTRAINT generated_always_as_history_entry_model_id_motion_id GENERATED ALWAYS AS (CASE WHEN split_part(model_id, '/', 1) = 'motion' THEN cast(split_part(model_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    model_id_assignment_id integer CONSTRAINT generated_always_as_history_entry_model_id_assignment_id GENERATED ALWAYS AS (CASE WHEN split_part(model_id, '/', 1) = 'assignment' THEN cast(split_part(model_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    CONSTRAINT valid_history_entry_model_id_part1 CHECK (split_part(model_id, '/', 1) IN ('user','motion','assignment')),
    position_id integer CONSTRAINT required_history_entry_position_id NOT NULL,
    meeting_id integer
);
...
CREATE TABLE import_preview_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_import_preview_name NOT NULL CONSTRAINT enum_import_preview_name CHECK (name IN ('account', 'participant', 'topic', 'committee', 'motion')),
    state varchar(256) CONSTRAINT required_import_preview_state NOT NULL CONSTRAINT enum_import_preview_state CHECK (state IN ('warning', 'error', 'done')),
    created timestamptz CONSTRAINT required_import_preview_created NOT NULL,
    result jsonb
);
...
CREATE TABLE list_of_speakers_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    closed boolean CONSTRAINT default_list_of_speakers_closed DEFAULT False,
    sequential_number integer CONSTRAINT required_list_of_speakers_sequential_number NOT NULL,
    CONSTRAINT unique_list_of_speakers_sequential_number_meeting_id UNIQUE (sequential_number, meeting_id),
    moderator_notes text,
    content_object_id varchar(100) CONSTRAINT required_list_of_speakers_content_object_id NOT NULL,
    content_object_id_motion_id integer CONSTRAINT unique_list_of_speakers_content_object_id_motion_id UNIQUE CONSTRAINT generated_always_as_list_of_speakers_content_object_id_m4822372 GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'motion' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_motion_block_id integer CONSTRAINT unique_list_of_speakers_content_object_id_motion_block_id UNIQUE CONSTRAINT generated_always_as_list_of_speakers_content_object_id_m50027ab GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'motion_block' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_assignment_id integer CONSTRAINT unique_list_of_speakers_content_object_id_assignment_id UNIQUE CONSTRAINT generated_always_as_list_of_speakers_content_object_id_abe56f76 GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'assignment' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_topic_id integer CONSTRAINT unique_list_of_speakers_content_object_id_topic_id UNIQUE CONSTRAINT generated_always_as_list_of_speakers_content_object_id_topic_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'topic' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_meeting_mediafile_id integer CONSTRAINT unique_list_of_speakers_content_object_id_meeting_mediafile_id UNIQUE CONSTRAINT generated_always_as_list_of_speakers_content_object_id_m07d8b8d GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'meeting_mediafile' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    ...
    meeting_id integer CONSTRAINT required_list_of_speakers_meeting_id NOT NULL
);
...
CREATE TABLE mediafile_t (
    ...
    parent_id integer,
    owner_id varchar(100) CONSTRAINT required_mediafile_owner_id NOT NULL,
    owner_id_meeting_id integer CONSTRAINT generated_always_as_mediafile_owner_id_meeting_id GENERATED ALWAYS AS (CASE WHEN split_part(owner_id, '/', 1) = 'meeting' THEN cast(split_part(owner_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    owner_id_organization_id integer CONSTRAINT generated_always_as_mediafile_owner_id_organization_id GENERATED ALWAYS AS (CASE WHEN split_part(owner_id, '/', 1) = 'organization' THEN cast(split_part(owner_id, '/', 2) AS INTEGER) ELSE null END) STORED,
...
CREATE TABLE meeting_t (
    ...
    welcome_title varchar(256) CONSTRAINT default_meeting_welcome_title DEFAULT 'Welcome to OpenSlides',
    welcome_text text CONSTRAINT default_meeting_welcome_text DEFAULT 'Space for your welcome text.',
    name varchar(200) CONSTRAINT required_meeting_name NOT NULL CONSTRAINT default_meeting_name DEFAULT 'OpenSlides',
    ...
    language varchar(256) CONSTRAINT enum_meeting_language CHECK (language IN ('en', 'de', 'it', 'es', 'ru', 'cs', 'fr')) CONSTRAINT default_meeting_language DEFAULT 'en',
    jitsi_domain varchar(256),
    jitsi_room_name varchar(256),
    jitsi_room_password varchar(256),
    template_for_organization_id integer,
    enable_anonymous boolean CONSTRAINT default_meeting_enable_anonymous DEFAULT False,
    custom_translations jsonb,
    conference_show boolean CONSTRAINT default_meeting_conference_show DEFAULT False,
    conference_auto_connect boolean CONSTRAINT default_meeting_conference_auto_connect DEFAULT False,
    conference_los_restriction boolean CONSTRAINT default_meeting_conference_los_restriction DEFAULT True,
    conference_stream_url varchar(256),
    conference_stream_poster_url varchar(256),
    conference_open_microphone boolean CONSTRAINT default_meeting_conference_open_microphone DEFAULT False,
    conference_open_video boolean CONSTRAINT default_meeting_conference_open_video DEFAULT False,
    conference_auto_connect_next_speakers integer CONSTRAINT default_meeting_conference_auto_connect_next_speakers DEFAULT 0,
    conference_enable_helpdesk boolean CONSTRAINT default_meeting_conference_enable_helpdesk DEFAULT False,
    applause_enable boolean CONSTRAINT default_meeting_applause_enable DEFAULT False,
    applause_type varchar(256) CONSTRAINT enum_meeting_applause_type CHECK (applause_type IN ('applause-type-bar', 'applause-type-particles')) CONSTRAINT default_meeting_applause_type DEFAULT 'applause-type-bar',
    applause_show_level boolean CONSTRAINT default_meeting_applause_show_level DEFAULT False,
    applause_min_amount integer CONSTRAINT minimum_meeting_applause_min_amount CHECK (applause_min_amount >= 0) CONSTRAINT default_meeting_applause_min_amount DEFAULT 1,
    applause_max_amount integer CONSTRAINT minimum_meeting_applause_max_amount CHECK (applause_max_amount >= 0) CONSTRAINT default_meeting_applause_max_amount DEFAULT 0,
    applause_timeout integer CONSTRAINT minimum_meeting_applause_timeout CHECK (applause_timeout >= 0) CONSTRAINT default_meeting_applause_timeout DEFAULT 5,
    applause_particle_image_url varchar(256),
    projector_countdown_default_time integer CONSTRAINT required_meeting_projector_countdown_default_time NOT NULL CONSTRAINT default_meeting_projector_countdown_default_time DEFAULT 60,
    projector_countdown_warning_time integer CONSTRAINT required_meeting_projector_countdown_warning_time NOT NULL CONSTRAINT minimum_meeting_projector_countdown_warning_time CHECK (projector_countdown_warning_time >= 0) CONSTRAINT default_meeting_projector_countdown_warning_time DEFAULT 0,
    export_csv_encoding varchar(256) CONSTRAINT enum_meeting_export_csv_encoding CHECK (export_csv_encoding IN ('utf-8', 'iso-8859-15')) CONSTRAINT default_meeting_export_csv_encoding DEFAULT 'utf-8',
    export_csv_separator varchar(256) CONSTRAINT default_meeting_export_csv_separator DEFAULT ';',
    export_pdf_pagenumber_alignment varchar(256) CONSTRAINT enum_meeting_export_pdf_pagenumber_alignment CHECK (export_pdf_pagenumber_alignment IN ('left', 'right', 'center')) CONSTRAINT default_meeting_export_pdf_pagenumber_alignment DEFAULT 'center',
    export_pdf_fontsize integer CONSTRAINT enum_meeting_export_pdf_fontsize CHECK (export_pdf_fontsize IN (10, 11, 12)) CONSTRAINT default_meeting_export_pdf_fontsize DEFAULT 10,
    export_pdf_line_height double precision CONSTRAINT minimum_meeting_export_pdf_line_height CHECK (export_pdf_line_height >= 1.0) CONSTRAINT default_meeting_export_pdf_line_height DEFAULT 1.25,
    export_pdf_page_margin_left integer CONSTRAINT minimum_meeting_export_pdf_page_margin_left CHECK (export_pdf_page_margin_left >= 0) CONSTRAINT default_meeting_export_pdf_page_margin_left DEFAULT 20,
    export_pdf_page_margin_top integer CONSTRAINT minimum_meeting_export_pdf_page_margin_top CHECK (export_pdf_page_margin_top >= 0) CONSTRAINT default_meeting_export_pdf_page_margin_top DEFAULT 25,
    export_pdf_page_margin_right integer CONSTRAINT minimum_meeting_export_pdf_page_margin_right CHECK (export_pdf_page_margin_right >= 0) CONSTRAINT default_meeting_export_pdf_page_margin_right DEFAULT 20,
    export_pdf_page_margin_bottom integer CONSTRAINT minimum_meeting_export_pdf_page_margin_bottom CHECK (export_pdf_page_margin_bottom >= 0) CONSTRAINT default_meeting_export_pdf_page_margin_bottom DEFAULT 20,
    export_pdf_pagesize varchar(256) CONSTRAINT enum_meeting_export_pdf_pagesize CHECK (export_pdf_pagesize IN ('A4', 'A5')) CONSTRAINT default_meeting_export_pdf_pagesize DEFAULT 'A4',
    agenda_show_subtitles boolean CONSTRAINT default_meeting_agenda_show_subtitles DEFAULT False,
    agenda_enable_numbering boolean CONSTRAINT default_meeting_agenda_enable_numbering DEFAULT True,
    agenda_number_prefix varchar(20),
    agenda_numeral_system varchar(256) CONSTRAINT enum_meeting_agenda_numeral_system CHECK (agenda_numeral_system IN ('arabic', 'roman')) CONSTRAINT default_meeting_agenda_numeral_system DEFAULT 'arabic',
    agenda_item_creation varchar(256) CONSTRAINT enum_meeting_agenda_item_creation CHECK (agenda_item_creation IN ('always', 'never', 'default_yes', 'default_no')) CONSTRAINT default_meeting_agenda_item_creation DEFAULT 'default_no',
    agenda_new_items_default_visibility varchar(256) CONSTRAINT enum_meeting_agenda_new_items_default_visibility CHECK (agenda_new_items_default_visibility IN ('common', 'internal', 'hidden')) CONSTRAINT default_meeting_agenda_new_items_default_visibility DEFAULT 'internal',
    agenda_show_internal_items_on_projector boolean CONSTRAINT default_meeting_agenda_show_internal_items_on_projector DEFAULT False,
    agenda_show_topic_navigation_on_detail_view boolean CONSTRAINT default_meeting_agenda_show_topic_navigation_on_detail_view DEFAULT False,
    list_of_speakers_amount_last_on_projector integer CONSTRAINT minimum_meeting_list_of_speakers_amount_last_on_projector CHECK (list_of_speakers_amount_last_on_projector >= -1) CONSTRAINT default_meeting_list_of_speakers_amount_last_on_projector DEFAULT 0,
    list_of_speakers_amount_next_on_projector integer CONSTRAINT minimum_meeting_list_of_speakers_amount_next_on_projector CHECK (list_of_speakers_amount_next_on_projector >= -1) CONSTRAINT default_meeting_list_of_speakers_amount_next_on_projector DEFAULT -1,
    list_of_speakers_couple_countdown boolean CONSTRAINT default_meeting_list_of_speakers_couple_countdown DEFAULT True,
    list_of_speakers_show_amount_of_speakers_on_slide boolean CONSTRAINT default_meeting_list_of_speakers_show_amount_of_speakersebf4b44 DEFAULT True,
    list_of_speakers_present_users_only boolean CONSTRAINT default_meeting_list_of_speakers_present_users_only DEFAULT False,
    list_of_speakers_show_first_contribution boolean CONSTRAINT default_meeting_list_of_speakers_show_first_contribution DEFAULT False,
    list_of_speakers_hide_contribution_count boolean CONSTRAINT default_meeting_list_of_speakers_hide_contribution_count DEFAULT False,
    list_of_speakers_allow_multiple_speakers boolean CONSTRAINT default_meeting_list_of_speakers_allow_multiple_speakers DEFAULT False,
    list_of_speakers_enable_point_of_order_speakers boolean CONSTRAINT default_meeting_list_of_speakers_enable_point_of_order_speakers DEFAULT True,
    list_of_speakers_can_create_point_of_order_for_others boolean CONSTRAINT default_meeting_list_of_speakers_can_create_point_of_ord50d63fe DEFAULT False,
    list_of_speakers_enable_point_of_order_categories boolean CONSTRAINT default_meeting_list_of_speakers_enable_point_of_order_c1167331 DEFAULT False,
    list_of_speakers_closing_disables_point_of_order boolean CONSTRAINT default_meeting_list_of_speakers_closing_disables_point_c59619b DEFAULT False,
    list_of_speakers_enable_pro_contra_speech boolean CONSTRAINT default_meeting_list_of_speakers_enable_pro_contra_speech DEFAULT False,
    list_of_speakers_can_set_contribution_self boolean CONSTRAINT default_meeting_list_of_speakers_can_set_contribution_self DEFAULT False,
    list_of_speakers_speaker_note_for_everyone boolean CONSTRAINT default_meeting_list_of_speakers_speaker_note_for_everyone DEFAULT True,
    list_of_speakers_initially_closed boolean CONSTRAINT default_meeting_list_of_speakers_initially_closed DEFAULT False,
    list_of_speakers_default_structure_level_time integer CONSTRAINT minimum_meeting_list_of_speakers_default_structure_level_time CHECK (list_of_speakers_default_structure_level_time >= 0),
    list_of_speakers_enable_interposed_question boolean,
    list_of_speakers_intervention_time integer,
    motions_default_workflow_id integer CONSTRAINT required_meeting_motions_default_workflow_id NOT NULL CONSTRAINT unique_meeting_motions_default_workflow_id UNIQUE,
    motions_default_amendment_workflow_id integer CONSTRAINT required_meeting_motions_default_amendment_workflow_id NOT NULL CONSTRAINT unique_meeting_motions_default_amendment_workflow_id UNIQUE,
    motions_preamble text CONSTRAINT default_meeting_motions_preamble DEFAULT 'The assembly may decide:',
    motions_default_line_numbering varchar(256) CONSTRAINT enum_meeting_motions_default_line_numbering CHECK (motions_default_line_numbering IN ('outside', 'inline', 'none')) CONSTRAINT default_meeting_motions_default_line_numbering DEFAULT 'outside',
    motions_line_length integer CONSTRAINT minimum_meeting_motions_line_length CHECK (motions_line_length >= 40) CONSTRAINT default_meeting_motions_line_length DEFAULT 85,
    motions_reason_required boolean CONSTRAINT default_meeting_motions_reason_required DEFAULT False,
    motions_origin_motion_toggle_default boolean CONSTRAINT default_meeting_motions_origin_motion_toggle_default DEFAULT False,
    motions_enable_origin_motion_display boolean CONSTRAINT default_meeting_motions_enable_origin_motion_display DEFAULT False,
    motions_enable_text_on_projector boolean CONSTRAINT default_meeting_motions_enable_text_on_projector DEFAULT True,
    motions_enable_reason_on_projector boolean CONSTRAINT default_meeting_motions_enable_reason_on_projector DEFAULT False,
    motions_enable_sidebox_on_projector boolean CONSTRAINT default_meeting_motions_enable_sidebox_on_projector DEFAULT False,
    motions_enable_recommendation_on_projector boolean CONSTRAINT default_meeting_motions_enable_recommendation_on_projector DEFAULT True,
    motions_hide_metadata_background boolean CONSTRAINT default_meeting_motions_hide_metadata_background DEFAULT False,
    motions_show_referring_motions boolean CONSTRAINT default_meeting_motions_show_referring_motions DEFAULT True,
    motions_show_sequential_number boolean CONSTRAINT default_meeting_motions_show_sequential_number DEFAULT True,
    motions_create_enable_additional_submitter_text boolean,
    motions_recommendations_by varchar(256),
    motions_block_slide_columns integer CONSTRAINT minimum_meeting_motions_block_slide_columns CHECK (motions_block_slide_columns >= 1),
    motions_recommendation_text_mode varchar(256) CONSTRAINT enum_meeting_motions_recommendation_text_mode CHECK (motions_recommendation_text_mode IN ('original', 'changed', 'diff', 'agreed')) CONSTRAINT default_meeting_motions_recommendation_text_mode DEFAULT 'diff',
    motions_default_sorting varchar(256) CONSTRAINT enum_meeting_motions_default_sorting CHECK (motions_default_sorting IN ('number', 'weight')) CONSTRAINT default_meeting_motions_default_sorting DEFAULT 'number',
    motions_number_type varchar(256) CONSTRAINT enum_meeting_motions_number_type CHECK (motions_number_type IN ('per_category', 'serially_numbered', 'manually')) CONSTRAINT default_meeting_motions_number_type DEFAULT 'per_category',
    motions_number_min_digits integer CONSTRAINT default_meeting_motions_number_min_digits DEFAULT 2,
    motions_number_with_blank boolean CONSTRAINT default_meeting_motions_number_with_blank DEFAULT False,
    motions_amendments_enabled boolean CONSTRAINT default_meeting_motions_amendments_enabled DEFAULT True,
    motions_amendments_in_main_list boolean CONSTRAINT default_meeting_motions_amendments_in_main_list DEFAULT True,
    motions_amendments_of_amendments boolean CONSTRAINT default_meeting_motions_amendments_of_amendments DEFAULT False,
    motions_amendments_prefix varchar(256) CONSTRAINT default_meeting_motions_amendments_prefix DEFAULT '-Ä',
    motions_amendments_text_mode varchar(256) CONSTRAINT enum_meeting_motions_amendments_text_mode CHECK (motions_amendments_text_mode IN ('freestyle', 'fulltext', 'paragraph')) CONSTRAINT default_meeting_motions_amendments_text_mode DEFAULT 'paragraph',
    motions_amendments_multiple_paragraphs boolean CONSTRAINT default_meeting_motions_amendments_multiple_paragraphs DEFAULT True,
    motions_supporters_min_amount integer CONSTRAINT minimum_meeting_motions_supporters_min_amount CHECK (motions_supporters_min_amount >= 0) CONSTRAINT default_meeting_motions_supporters_min_amount DEFAULT 0,
    motions_enable_editor boolean,
    motions_enable_working_group_speaker boolean,
    motions_export_title varchar(256) CONSTRAINT default_meeting_motions_export_title DEFAULT 'Motions',
    motions_export_preamble text,
    motions_export_submitter_recommendation boolean CONSTRAINT default_meeting_motions_export_submitter_recommendation DEFAULT True,
    motions_export_follow_recommendation boolean CONSTRAINT default_meeting_motions_export_follow_recommendation DEFAULT False,
    motions_enable_restricted_editor_for_manager boolean,
    motions_enable_restricted_editor_for_non_manager boolean,
    motion_poll_ballot_paper_selection varchar(256) CONSTRAINT enum_meeting_motion_poll_ballot_paper_selection CHECK (motion_poll_ballot_paper_selection IN ('NUMBER_OF_DELEGATES', 'NUMBER_OF_ALL_PARTICIPANTS', 'CUSTOM_NUMBER')) CONSTRAINT default_meeting_motion_poll_ballot_paper_selection DEFAULT 'CUSTOM_NUMBER',
    motion_poll_ballot_paper_number integer CONSTRAINT default_meeting_motion_poll_ballot_paper_number DEFAULT 8,
    motion_poll_default_type varchar(256) CONSTRAINT default_meeting_motion_poll_default_type DEFAULT 'pseudoanonymous',
    motion_poll_default_method varchar(256) CONSTRAINT default_meeting_motion_poll_default_method DEFAULT 'YNA',
    motion_poll_default_onehundred_percent_base varchar(256) CONSTRAINT enum_meeting_motion_poll_default_onehundred_percent_base CHECK (motion_poll_default_onehundred_percent_base IN ('Y', 'YN', 'YNA', 'N', 'valid', 'cast', 'entitled', 'entitled_present', 'disabled')) CONSTRAINT default_meeting_motion_poll_default_onehundred_percent_base DEFAULT 'YNA',
    motion_poll_default_backend varchar(256) CONSTRAINT enum_meeting_motion_poll_default_backend CHECK (motion_poll_default_backend IN ('long', 'fast')) CONSTRAINT default_meeting_motion_poll_default_backend DEFAULT 'fast',
    motion_poll_projection_name_order_first varchar(256) CONSTRAINT required_meeting_motion_poll_projection_name_order_first NOT NULL CONSTRAINT enum_meeting_motion_poll_projection_name_order_first CHECK (motion_poll_projection_name_order_first IN ('first_name', 'last_name')) CONSTRAINT default_meeting_motion_poll_projection_name_order_first DEFAULT 'last_name',
    motion_poll_projection_max_columns integer CONSTRAINT required_meeting_motion_poll_projection_max_columns NOT NULL CONSTRAINT default_meeting_motion_poll_projection_max_columns DEFAULT 6,
    users_enable_presence_view boolean CONSTRAINT default_meeting_users_enable_presence_view DEFAULT False,
    users_enable_vote_weight boolean CONSTRAINT default_meeting_users_enable_vote_weight DEFAULT False,
    users_allow_self_set_present boolean CONSTRAINT default_meeting_users_allow_self_set_present DEFAULT True,
    users_pdf_welcometitle varchar(256) CONSTRAINT default_meeting_users_pdf_welcometitle DEFAULT 'Welcome to OpenSlides',
    users_pdf_welcometext text CONSTRAINT default_meeting_users_pdf_welcometext DEFAULT '[Place for your welcome and help text.]',
    users_pdf_wlan_ssid varchar(256),
    users_pdf_wlan_password varchar(256),
    users_pdf_wlan_encryption varchar(256) CONSTRAINT enum_meeting_users_pdf_wlan_encryption CHECK (users_pdf_wlan_encryption IN ('', 'WEP', 'WPA', 'nopass')) CONSTRAINT default_meeting_users_pdf_wlan_encryption DEFAULT 'WPA',
    users_email_sender varchar(256) CONSTRAINT default_meeting_users_email_sender DEFAULT 'OpenSlides',
    users_email_replyto varchar(256),
    users_email_subject varchar(256) CONSTRAINT default_meeting_users_email_subject DEFAULT 'OpenSlides access data',
    users_email_body text CONSTRAINT default_meeting_users_email_body DEFAULT 'Dear {name},
...
This email was generated automatically.',
    ...
    assignments_export_title varchar(256) CONSTRAINT default_meeting_assignments_export_title DEFAULT 'Elections',
    assignments_export_preamble text,
    assignment_poll_ballot_paper_selection varchar(256) CONSTRAINT enum_meeting_assignment_poll_ballot_paper_selection CHECK (assignment_poll_ballot_paper_selection IN ('NUMBER_OF_DELEGATES', 'NUMBER_OF_ALL_PARTICIPANTS', 'CUSTOM_NUMBER')) CONSTRAINT default_meeting_assignment_poll_ballot_paper_selection DEFAULT 'CUSTOM_NUMBER',
    assignment_poll_ballot_paper_number integer CONSTRAINT default_meeting_assignment_poll_ballot_paper_number DEFAULT 8,
    assignment_poll_add_candidates_to_list_of_speakers boolean CONSTRAINT default_meeting_assignment_poll_add_candidates_to_list_od04213d DEFAULT False,
    assignment_poll_enable_max_votes_per_option boolean CONSTRAINT default_meeting_assignment_poll_enable_max_votes_per_option DEFAULT False,
    assignment_poll_sort_poll_result_by_votes boolean CONSTRAINT default_meeting_assignment_poll_sort_poll_result_by_votes DEFAULT True,
    assignment_poll_default_type varchar(256) CONSTRAINT default_meeting_assignment_poll_default_type DEFAULT 'pseudoanonymous',
    assignment_poll_default_method varchar(256) CONSTRAINT default_meeting_assignment_poll_default_method DEFAULT 'Y',
    assignment_poll_default_onehundred_percent_base varchar(256) CONSTRAINT enum_meeting_assignment_poll_default_onehundred_percent_base CHECK (assignment_poll_default_onehundred_percent_base IN ('Y', 'YN', 'YNA', 'N', 'valid', 'cast', 'entitled', 'entitled_present', 'disabled')) CONSTRAINT default_meeting_assignment_poll_default_onehundred_percent_base DEFAULT 'valid',
    assignment_poll_default_backend varchar(256) CONSTRAINT enum_meeting_assignment_poll_default_backend CHECK (assignment_poll_default_backend IN ('long', 'fast')) CONSTRAINT default_meeting_assignment_poll_default_backend DEFAULT 'fast',
    ...
    poll_default_type varchar(256) CONSTRAINT default_meeting_poll_default_type DEFAULT 'analog',
    poll_default_method varchar(256),
    poll_default_onehundred_percent_base varchar(256) CONSTRAINT enum_meeting_poll_default_onehundred_percent_base CHECK (poll_default_onehundred_percent_base IN ('Y', 'YN', 'YNA', 'N', 'valid', 'cast', 'entitled', 'entitled_present', 'disabled')) CONSTRAINT default_meeting_poll_default_onehundred_percent_base DEFAULT 'YNA',
    poll_default_backend varchar(256) CONSTRAINT enum_meeting_poll_default_backend CHECK (poll_default_backend IN ('long', 'fast')) CONSTRAINT default_meeting_poll_default_backend DEFAULT 'fast',
    poll_default_live_voting_enabled boolean CONSTRAINT default_meeting_poll_default_live_voting_enabled DEFAULT False,
    poll_couple_countdown boolean CONSTRAINT default_meeting_poll_couple_countdown DEFAULT True,
    ...
    committee_id integer CONSTRAINT required_meeting_committee_id NOT NULL,
    reference_projector_id integer CONSTRAINT required_meeting_reference_projector_id NOT NULL CONSTRAINT unique_meeting_reference_projector_id UNIQUE,
    ...
    default_group_id integer CONSTRAINT required_meeting_default_group_id NOT NULL CONSTRAINT unique_meeting_default_group_id UNIQUE,
...
CREATE TABLE meeting_mediafile_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    mediafile_id integer CONSTRAINT required_meeting_mediafile_mediafile_id NOT NULL,
    meeting_id integer CONSTRAINT required_meeting_mediafile_meeting_id NOT NULL,
    is_public boolean CONSTRAINT required_meeting_mediafile_is_public NOT NULL
);
...
CREATE TABLE meeting_user_t (
    ...
    number varchar(256),
    about_me text,
    vote_weight decimal(16,6) CONSTRAINT minimum_meeting_user_vote_weight CHECK (vote_weight >= 0.000001),
    locked_out boolean,
    user_id integer CONSTRAINT required_meeting_user_user_id NOT NULL,
    meeting_id integer CONSTRAINT required_meeting_user_meeting_id NOT NULL,
...
CREATE TABLE motion_t (
    ...
    sequential_number integer CONSTRAINT required_motion_sequential_number NOT NULL,
    CONSTRAINT unique_motion_sequential_number_meeting_id UNIQUE (sequential_number, meeting_id),
    title varchar(256) CONSTRAINT required_motion_title NOT NULL,
    ...
    reason text,
    category_weight integer CONSTRAINT default_motion_category_weight DEFAULT 10000,
    state_extension varchar(256),
    recommendation_extension varchar(256),
    sort_weight integer CONSTRAINT default_motion_sort_weight DEFAULT 10000,
    created timestamptz,
    last_modified timestamptz,
    workflow_timestamp timestamptz,
    start_line_number integer CONSTRAINT minimum_motion_start_line_number CHECK (start_line_number >= 1) CONSTRAINT default_motion_start_line_number DEFAULT 1,
    ...
    state_id integer CONSTRAINT required_motion_state_id NOT NULL,
    recommendation_id integer,
    category_id integer,
    block_id integer,
    meeting_id integer CONSTRAINT required_motion_meeting_id NOT NULL,
...
CREATE TABLE motion_block_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    title varchar(256) CONSTRAINT required_motion_block_title NOT NULL,
    internal boolean,
    sequential_number integer CONSTRAINT required_motion_block_sequential_number NOT NULL,
    CONSTRAINT unique_motion_block_sequential_number_meeting_id UNIQUE (sequential_number, meeting_id),
    meeting_id integer CONSTRAINT required_motion_block_meeting_id NOT NULL
);
...
CREATE TABLE motion_category_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_motion_category_name NOT NULL,
    prefix varchar(256),
    weight integer CONSTRAINT default_motion_category_weight DEFAULT 10000,
    level integer,
    sequential_number integer CONSTRAINT required_motion_category_sequential_number NOT NULL,
    CONSTRAINT unique_motion_category_sequential_number_meeting_id UNIQUE (sequential_number, meeting_id),
    parent_id integer,
    meeting_id integer CONSTRAINT required_motion_category_meeting_id NOT NULL
);
...
CREATE TABLE motion_change_recommendation_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    rejected boolean CONSTRAINT default_motion_change_recommendation_rejected DEFAULT False,
    internal boolean CONSTRAINT default_motion_change_recommendation_internal DEFAULT False,
    type varchar(256) CONSTRAINT enum_motion_change_recommendation_type CHECK (type IN ('replacement', 'insertion', 'deletion', 'other')) CONSTRAINT default_motion_change_recommendation_type DEFAULT 'replacement',
    other_description varchar(256),
    line_from integer CONSTRAINT minimum_motion_change_recommendation_line_from CHECK (line_from >= 0),
    line_to integer CONSTRAINT minimum_motion_change_recommendation_line_to CHECK (line_to >= 0),
    text text,
    creation_time timestamptz,
    motion_id integer CONSTRAINT required_motion_change_recommendation_motion_id NOT NULL,
    meeting_id integer CONSTRAINT required_motion_change_recommendation_meeting_id NOT NULL
);
...
CREATE TABLE motion_comment_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    comment text,
    motion_id integer CONSTRAINT required_motion_comment_motion_id NOT NULL,
    section_id integer CONSTRAINT required_motion_comment_section_id NOT NULL,
    meeting_id integer CONSTRAINT required_motion_comment_meeting_id NOT NULL,
...
CREATE TABLE motion_comment_section_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_motion_comment_section_name NOT NULL,
    weight integer CONSTRAINT default_motion_comment_section_weight DEFAULT 10000,
    sequential_number integer CONSTRAINT required_motion_comment_section_sequential_number NOT NULL,
    ...
    meeting_id integer CONSTRAINT required_motion_comment_section_meeting_id NOT NULL
);
...
CREATE TABLE motion_editor_t (
    ...
    weight integer,
    meeting_user_id integer,
    motion_id integer CONSTRAINT required_motion_editor_motion_id NOT NULL,
    meeting_id integer CONSTRAINT required_motion_editor_meeting_id NOT NULL,
...
CREATE TABLE motion_state_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_motion_state_name NOT NULL,
    weight integer CONSTRAINT required_motion_state_weight NOT NULL,
    recommendation_label varchar(256),
    is_internal boolean,
    css_class varchar(256) CONSTRAINT required_motion_state_css_class NOT NULL CONSTRAINT enum_motion_state_css_class CHECK (css_class IN ('grey', 'red', 'green', 'lightblue', 'yellow')) CONSTRAINT default_motion_state_css_class DEFAULT 'lightblue',
    restrictions varchar(256)[] CONSTRAINT enum_motion_state_restrictions CHECK (restrictions <@ ARRAY['motion.can_see_internal', 'motion.can_manage_metadata', 'motion.can_manage', 'is_submitter']::varchar[]) CONSTRAINT default_motion_state_restrictions DEFAULT '{}',
    allow_support boolean CONSTRAINT default_motion_state_allow_support DEFAULT False,
    allow_create_poll boolean CONSTRAINT default_motion_state_allow_create_poll DEFAULT False,
    allow_submitter_edit boolean CONSTRAINT default_motion_state_allow_submitter_edit DEFAULT False,
    set_number boolean CONSTRAINT default_motion_state_set_number DEFAULT True,
    show_state_extension_field boolean CONSTRAINT default_motion_state_show_state_extension_field DEFAULT False,
    show_recommendation_extension_field boolean CONSTRAINT default_motion_state_show_recommendation_extension_field DEFAULT False,
    merge_amendment_into_final varchar(256) CONSTRAINT enum_motion_state_merge_amendment_into_final CHECK (merge_amendment_into_final IN ('do_not_merge', 'undefined', 'do_merge')) CONSTRAINT default_motion_state_merge_amendment_into_final DEFAULT 'undefined',
    allow_motion_forwarding boolean CONSTRAINT default_motion_state_allow_motion_forwarding DEFAULT False,
    allow_amendment_forwarding boolean,
    set_workflow_timestamp boolean CONSTRAINT default_motion_state_set_workflow_timestamp DEFAULT False,
    state_button_label varchar(256),
    submitter_withdraw_state_id integer,
    workflow_id integer CONSTRAINT required_motion_state_workflow_id NOT NULL,
    meeting_id integer CONSTRAINT required_motion_state_meeting_id NOT NULL,
...
CREATE TABLE motion_submitter_t (
    ...
    weight integer,
    meeting_user_id integer,
    motion_id integer CONSTRAINT required_motion_submitter_motion_id NOT NULL,
    meeting_id integer CONSTRAINT required_motion_submitter_meeting_id NOT NULL,
...
CREATE TABLE motion_supporter_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    meeting_user_id integer,
    motion_id integer CONSTRAINT required_motion_supporter_motion_id NOT NULL,
    meeting_id integer CONSTRAINT required_motion_supporter_meeting_id NOT NULL,
...
CREATE TABLE motion_workflow_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_motion_workflow_name NOT NULL,
    sequential_number integer CONSTRAINT required_motion_workflow_sequential_number NOT NULL,
    CONSTRAINT unique_motion_workflow_sequential_number_meeting_id UNIQUE (sequential_number, meeting_id),
    first_state_id integer CONSTRAINT required_motion_workflow_first_state_id NOT NULL CONSTRAINT unique_motion_workflow_first_state_id UNIQUE,
    meeting_id integer CONSTRAINT required_motion_workflow_meeting_id NOT NULL
);
...
CREATE TABLE motion_working_group_speaker_t (
    ...
    weight integer,
    meeting_user_id integer,
    motion_id integer CONSTRAINT required_motion_working_group_speaker_motion_id NOT NULL,
    meeting_id integer CONSTRAINT required_motion_working_group_speaker_meeting_id NOT NULL,
...
CREATE TABLE option_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    weight integer CONSTRAINT default_option_weight DEFAULT 10000,
    text text,
    yes decimal(16,6),
    no decimal(16,6),
    ...
    content_object_id_motion_id integer CONSTRAINT generated_always_as_option_content_object_id_motion_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'motion' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_user_id integer CONSTRAINT generated_always_as_option_content_object_id_user_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'user' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_poll_candidate_list_id integer CONSTRAINT unique_option_content_object_id_poll_candidate_list_id UNIQUE CONSTRAINT generated_always_as_option_content_object_id_poll_candidd7449d9 GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'poll_candidate_list' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    CONSTRAINT valid_option_content_object_id_part1 CHECK (split_part(content_object_id, '/', 1) IN ('motion','user','poll_candidate_list')),
    meeting_id integer CONSTRAINT required_option_meeting_id NOT NULL,
...
CREATE TABLE organization_t (
    ...
    enable_chat boolean,
    limit_of_meetings integer CONSTRAINT minimum_organization_limit_of_meetings CHECK (limit_of_meetings >= 0) CONSTRAINT default_organization_limit_of_meetings DEFAULT 0,
    limit_of_users integer CONSTRAINT minimum_organization_limit_of_users CHECK (limit_of_users >= 0) CONSTRAINT default_organization_limit_of_users DEFAULT 0,
    default_language varchar(256) CONSTRAINT enum_organization_default_language CHECK (default_language IN ('en', 'de', 'it', 'es', 'ru', 'cs', 'fr')) CONSTRAINT default_organization_default_language DEFAULT 'en',
    ...
    saml_login_button_text varchar(256) CONSTRAINT default_organization_saml_login_button_text DEFAULT 'SAML login',
    saml_attr_mapping jsonb,
    saml_metadata_idp text,
    saml_metadata_sp text,
    saml_private_key text,
    theme_id integer CONSTRAINT required_organization_theme_id NOT NULL CONSTRAINT unique_organization_theme_id UNIQUE,
    users_email_sender varchar(256) CONSTRAINT default_organization_users_email_sender DEFAULT 'OpenSlides',
    users_email_replyto varchar(256),
    users_email_subject varchar(256) CONSTRAINT default_organization_users_email_subject DEFAULT 'OpenSlides access data',
    users_email_body text CONSTRAINT default_organization_users_email_body DEFAULT 'Dear {name},
...
This email was generated automatically.',
    url varchar(256) CONSTRAINT default_organization_url DEFAULT 'https://example.com'
);
...
CREATE TABLE organization_tag_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_organization_tag_name NOT NULL,
    color varchar(7) CHECK (color is null or color ~* '^#[a-f0-9]{6}$') CONSTRAINT required_organization_tag_color NOT NULL,
    organization_id integer CONSTRAINT required_organization_tag_organization_id NOT NULL CONSTRAINT default_organization_tag_organization_id DEFAULT 1
);
...
CREATE TABLE personal_note_t (
    ...
    note text,
    star boolean,
    meeting_user_id integer CONSTRAINT required_personal_note_meeting_user_id NOT NULL,
    content_object_id varchar(100),
    content_object_id_motion_id integer CONSTRAINT generated_always_as_personal_note_content_object_id_motion_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'motion' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    CONSTRAINT valid_personal_note_content_object_id_part1 CHECK (split_part(content_object_id, '/', 1) IN ('motion')),
    meeting_id integer CONSTRAINT required_personal_note_meeting_id NOT NULL,
...
CREATE TABLE point_of_order_category_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    text varchar(256) CONSTRAINT required_point_of_order_category_text NOT NULL,
    rank integer CONSTRAINT required_point_of_order_category_rank NOT NULL,
    meeting_id integer CONSTRAINT required_point_of_order_category_meeting_id NOT NULL
);
...
CREATE TABLE poll_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    title varchar(256) CONSTRAINT required_poll_title NOT NULL,
    description varchar(256),
    type varchar(256) CONSTRAINT required_poll_type NOT NULL CONSTRAINT enum_poll_type CHECK (type IN ('analog', 'named', 'pseudoanonymous', 'cryptographic')),
    backend varchar(256) CONSTRAINT required_poll_backend NOT NULL CONSTRAINT enum_poll_backend CHECK (backend IN ('long', 'fast')) CONSTRAINT default_poll_backend DEFAULT 'fast',
    is_pseudoanonymized boolean,
    pollmethod varchar(256) CONSTRAINT required_poll_pollmethod NOT NULL CONSTRAINT enum_poll_pollmethod CHECK (pollmethod IN ('Y', 'YN', 'YNA', 'N')),
    state varchar(256) CONSTRAINT enum_poll_state CHECK (state IN ('created', 'started', 'finished', 'published')) CONSTRAINT default_poll_state DEFAULT 'created',
    min_votes_amount integer CONSTRAINT minimum_poll_min_votes_amount CHECK (min_votes_amount >= 1) CONSTRAINT default_poll_min_votes_amount DEFAULT 1,
    max_votes_amount integer CONSTRAINT minimum_poll_max_votes_amount CHECK (max_votes_amount >= 1) CONSTRAINT default_poll_max_votes_amount DEFAULT 1,
    max_votes_per_option integer CONSTRAINT minimum_poll_max_votes_per_option CHECK (max_votes_per_option >= 1) CONSTRAINT default_poll_max_votes_per_option DEFAULT 1,
    global_yes boolean CONSTRAINT default_poll_global_yes DEFAULT False,
    global_no boolean CONSTRAINT default_poll_global_no DEFAULT False,
    global_abstain boolean CONSTRAINT default_poll_global_abstain DEFAULT False,
    onehundred_percent_base varchar(256) CONSTRAINT required_poll_onehundred_percent_base NOT NULL CONSTRAINT enum_poll_onehundred_percent_base CHECK (onehundred_percent_base IN ('Y', 'YN', 'YNA', 'N', 'valid', 'cast', 'entitled', 'entitled_present', 'disabled')) CONSTRAINT default_poll_onehundred_percent_base DEFAULT 'disabled',
    votesvalid decimal(16,6),
    votesinvalid decimal(16,6),
    votescast decimal(16,6),
    entitled_users_at_stop jsonb,
    live_voting_enabled boolean CONSTRAINT default_poll_live_voting_enabled DEFAULT False,
    sequential_number integer CONSTRAINT required_poll_sequential_number NOT NULL,
    CONSTRAINT unique_poll_sequential_number_meeting_id UNIQUE (sequential_number, meeting_id),
    content_object_id varchar(100) CONSTRAINT required_poll_content_object_id NOT NULL,
    content_object_id_motion_id integer CONSTRAINT generated_always_as_poll_content_object_id_motion_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'motion' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_assignment_id integer CONSTRAINT generated_always_as_poll_content_object_id_assignment_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'assignment' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_topic_id integer CONSTRAINT generated_always_as_poll_content_object_id_topic_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'topic' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    ...
    meeting_id integer CONSTRAINT required_poll_meeting_id NOT NULL
);
...
CREATE TABLE poll_candidate_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    poll_candidate_list_id integer CONSTRAINT required_poll_candidate_poll_candidate_list_id NOT NULL,
    user_id integer,
    weight integer CONSTRAINT required_poll_candidate_weight NOT NULL,
    meeting_id integer CONSTRAINT required_poll_candidate_meeting_id NOT NULL
);
...
CREATE TABLE poll_candidate_list_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    meeting_id integer CONSTRAINT required_poll_candidate_list_meeting_id NOT NULL
);
...
CREATE TABLE projection_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    options jsonb,
    stable boolean CONSTRAINT default_projection_stable DEFAULT False,
    weight integer,
    type varchar(256),
    current_projector_id integer,
    preview_projector_id integer,
    history_projector_id integer,
    content_object_id varchar(100) CONSTRAINT required_projection_content_object_id NOT NULL,
    content_object_id_meeting_id integer CONSTRAINT generated_always_as_projection_content_object_id_meeting_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'meeting' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_motion_id integer CONSTRAINT generated_always_as_projection_content_object_id_motion_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'motion' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_meeting_mediafile_id integer CONSTRAINT generated_always_as_projection_content_object_id_meeting51b9977 GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'meeting_mediafile' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_list_of_speakers_id integer CONSTRAINT generated_always_as_projection_content_object_id_list_of6e51c6f GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'list_of_speakers' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_motion_block_id integer CONSTRAINT generated_always_as_projection_content_object_id_motion_31fedd3 GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'motion_block' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_assignment_id integer CONSTRAINT generated_always_as_projection_content_object_id_assignment_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'assignment' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_agenda_item_id integer CONSTRAINT generated_always_as_projection_content_object_id_agenda_item_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'agenda_item' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_topic_id integer CONSTRAINT generated_always_as_projection_content_object_id_topic_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'topic' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_poll_id integer CONSTRAINT generated_always_as_projection_content_object_id_poll_id GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'poll' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_projector_message_id integer CONSTRAINT generated_always_as_projection_content_object_id_project05617d4 GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'projector_message' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    content_object_id_projector_countdown_id integer CONSTRAINT generated_always_as_projection_content_object_id_project708e3f9 GENERATED ALWAYS AS (CASE WHEN split_part(content_object_id, '/', 1) = 'projector_countdown' THEN cast(split_part(content_object_id, '/', 2) AS INTEGER) ELSE null END) STORED,
    ...
    meeting_id integer CONSTRAINT required_projection_meeting_id NOT NULL
);
...
CREATE TABLE projector_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256),
    is_internal boolean CONSTRAINT default_projector_is_internal DEFAULT False,
    scale integer CONSTRAINT default_projector_scale DEFAULT 0,
    scroll integer CONSTRAINT minimum_projector_scroll CHECK (scroll >= 0) CONSTRAINT default_projector_scroll DEFAULT 0,
    width integer CONSTRAINT minimum_projector_width CHECK (width >= 1) CONSTRAINT default_projector_width DEFAULT 1200,
    aspect_ratio_numerator integer CONSTRAINT minimum_projector_aspect_ratio_numerator CHECK (aspect_ratio_numerator >= 1) CONSTRAINT default_projector_aspect_ratio_numerator DEFAULT 16,
    aspect_ratio_denominator integer CONSTRAINT minimum_projector_aspect_ratio_denominator CHECK (aspect_ratio_denominator >= 1) CONSTRAINT default_projector_aspect_ratio_denominator DEFAULT 9,
    color varchar(7) CHECK (color is null or color ~* '^#[a-f0-9]{6}$') CONSTRAINT default_projector_color DEFAULT '#000000',
    background_color varchar(7) CHECK (background_color is null or background_color ~* '^#[a-f0-9]{6}$') CONSTRAINT default_projector_background_color DEFAULT '#ffffff',
    header_background_color varchar(7) CHECK (header_background_color is null or header_background_color ~* '^#[a-f0-9]{6}$') CONSTRAINT default_projector_header_background_color DEFAULT '#317796',
    header_font_color varchar(7) CHECK (header_font_color is null or header_font_color ~* '^#[a-f0-9]{6}$') CONSTRAINT default_projector_header_font_color DEFAULT '#f5f5f5',
    header_h1_color varchar(7) CHECK (header_h1_color is null or header_h1_color ~* '^#[a-f0-9]{6}$') CONSTRAINT default_projector_header_h1_color DEFAULT '#317796',
    chyron_background_color varchar(7) CHECK (chyron_background_color is null or chyron_background_color ~* '^#[a-f0-9]{6}$') CONSTRAINT default_projector_chyron_background_color DEFAULT '#317796',
    chyron_background_color_2 varchar(7) CHECK (chyron_background_color_2 is null or chyron_background_color_2 ~* '^#[a-f0-9]{6}$') CONSTRAINT default_projector_chyron_background_color_2 DEFAULT '#134768',
    chyron_font_color varchar(7) CHECK (chyron_font_color is null or chyron_font_color ~* '^#[a-f0-9]{6}$') CONSTRAINT default_projector_chyron_font_color DEFAULT '#ffffff',
    chyron_font_color_2 varchar(7) CHECK (chyron_font_color_2 is null or chyron_font_color_2 ~* '^#[a-f0-9]{6}$') CONSTRAINT default_projector_chyron_font_color_2 DEFAULT '#ffffff',
    show_header_footer boolean CONSTRAINT default_projector_show_header_footer DEFAULT True,
    show_title boolean CONSTRAINT default_projector_show_title DEFAULT True,
    show_logo boolean CONSTRAINT default_projector_show_logo DEFAULT True,
    show_clock boolean CONSTRAINT default_projector_show_clock DEFAULT True,
    sequential_number integer CONSTRAINT required_projector_sequential_number NOT NULL,
    ...
    meeting_id integer CONSTRAINT required_projector_meeting_id NOT NULL
);
...
CREATE TABLE projector_countdown_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    title varchar(256) CONSTRAINT required_projector_countdown_title NOT NULL,
    description varchar(256) CONSTRAINT default_projector_countdown_description DEFAULT '',
    default_time integer,
    countdown_time double precision CONSTRAINT default_projector_countdown_countdown_time DEFAULT 60,
    running boolean CONSTRAINT default_projector_countdown_running DEFAULT False,
    meeting_id integer CONSTRAINT required_projector_countdown_meeting_id NOT NULL,
...
CREATE TABLE projector_message_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    message text,
    meeting_id integer CONSTRAINT required_projector_message_meeting_id NOT NULL
);
...
CREATE TABLE speaker_t (
    ...
    total_pause integer,
    weight integer CONSTRAINT default_speaker_weight DEFAULT 10000,
    ...
    list_of_speakers_id integer CONSTRAINT required_speaker_list_of_speakers_id NOT NULL,
    structure_level_list_of_speakers_id integer,
    meeting_user_id integer,
    point_of_order_category_id integer,
    meeting_id integer CONSTRAINT required_speaker_meeting_id NOT NULL
);
...
CREATE TABLE structure_level_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_structure_level_name NOT NULL,
    color varchar(7) CHECK (color is null or color ~* '^#[a-f0-9]{6}$'),
    default_time integer CONSTRAINT minimum_structure_level_default_time CHECK (default_time >= 0),
    meeting_id integer CONSTRAINT required_structure_level_meeting_id NOT NULL,
...
CREATE TABLE structure_level_list_of_speakers_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    structure_level_id integer CONSTRAINT required_structure_level_list_of_speakers_structure_level_id NOT NULL,
    list_of_speakers_id integer CONSTRAINT required_structure_level_list_of_speakers_list_of_speakers_id NOT NULL,
    initial_time integer CONSTRAINT required_structure_level_list_of_speakers_initial_time NOT NULL CONSTRAINT minimum_structure_level_list_of_speakers_initial_time CHECK (initial_time >= 1),
    additional_time double precision,
    remaining_time double precision CONSTRAINT required_structure_level_list_of_speakers_remaining_time NOT NULL,
    current_start_time timestamptz,
    meeting_id integer CONSTRAINT required_structure_level_list_of_speakers_meeting_id NOT NULL,
...
CREATE TABLE tag_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_tag_name NOT NULL,
    meeting_id integer CONSTRAINT required_tag_meeting_id NOT NULL
);
...
CREATE TABLE theme_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name varchar(256) CONSTRAINT required_theme_name NOT NULL,
    ...
    accent_500 varchar(7) CHECK (accent_500 is null or accent_500 ~* '^#[a-f0-9]{6}$') CONSTRAINT default_theme_accent_500 DEFAULT '#2196f3',
    ...
    primary_500 varchar(7) CHECK (primary_500 is null or primary_500 ~* '^#[a-f0-9]{6}$') CONSTRAINT default_theme_primary_500 DEFAULT '#317796',
    ...
    warn_500 varchar(7) CHECK (warn_500 is null or warn_500 ~* '^#[a-f0-9]{6}$') CONSTRAINT default_theme_warn_500 DEFAULT '#f06400',
    ...
    organization_id integer CONSTRAINT required_theme_organization_id NOT NULL CONSTRAINT default_theme_organization_id DEFAULT 1
);
...
CREATE TABLE topic_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    title varchar(256) CONSTRAINT required_topic_title NOT NULL,
    text text,
    sequential_number integer CONSTRAINT required_topic_sequential_number NOT NULL,
    CONSTRAINT unique_topic_sequential_number_meeting_id UNIQUE (sequential_number, meeting_id),
    meeting_id integer CONSTRAINT required_topic_meeting_id NOT NULL
);
...
CREATE TABLE user_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    username varchar(256) CONSTRAINT required_user_username NOT NULL CONSTRAINT unique_user_username UNIQUE,
    member_number varchar(256) CONSTRAINT unique_user_member_number UNIQUE,
    saml_id varchar(256) CONSTRAINT unique_user_saml_id UNIQUE CONSTRAINT minlength_user_saml_id CHECK (char_length(saml_id) >= 1),
    pronoun varchar(32),
    title varchar(256),
    first_name varchar(256),
    last_name varchar(256),
    is_active boolean CONSTRAINT default_user_is_active DEFAULT True,
    is_physical_person boolean CONSTRAINT default_user_is_physical_person DEFAULT True,
    password varchar(256),
    default_password varchar(256),
    can_change_own_password boolean CONSTRAINT default_user_can_change_own_password DEFAULT True,
    email varchar(256),
    default_vote_weight decimal(16,6) CONSTRAINT minimum_user_default_vote_weight CHECK (default_vote_weight >= 0.000001) CONSTRAINT default_user_default_vote_weight DEFAULT '1.000000',
    ...
    organization_id integer CONSTRAINT required_user_organization_id NOT NULL CONSTRAINT default_user_organization_id DEFAULT 1
);
...
CREATE TABLE vote_t (
    ...
    weight decimal(16,6),
    value varchar(256),
    user_token varchar(256) CONSTRAINT required_vote_user_token NOT NULL,
    option_id integer CONSTRAINT required_vote_option_id NOT NULL,
    user_id integer,
    delegated_user_id integer,
    meeting_id integer CONSTRAINT required_vote_meeting_id NOT NULL
);
