...
-- MODELS_YML_CHECKSUM = '1138825df140632219d2a442c6c0d473'
...
CREATE TABLE nm_meeting_user_entitled_at_poll_ids_poll_entitled_user_t (
    meeting_user_id integer
        CONSTRAINT required_nm_meeting_user_entitled_at_poll_ids_poll_entitc50ead0 NOT NULL
        CONSTRAINT fk_nm_meeting_user_entitled_at_poll_ids_poll_entitled_usc5ea6cf REFERENCES meeting_user_t (id)
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    poll_entitled_user_id integer
        CONSTRAINT required_nm_meeting_user_entitled_at_poll_ids_poll_entit26d5cfc NOT NULL
        CONSTRAINT fk_nm_meeting_user_entitled_at_poll_ids_poll_entitled_us685fafc REFERENCES poll_entitled_user_t (id)
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_meeting_user_entitled_at_poll_ids_poll_entitled_user_t PRIMARY KEY (meeting_user_id, poll_entitled_user_id)
);
CREATE INDEX idx_nm_meeting_user_entitled_at_poll_ids_poll_entitled_u3739cf2 ON nm_meeting_user_entitled_at_poll_ids_poll_entitled_user_t (meeting_user_id);
CREATE INDEX idx_nm_meeting_user_entitled_at_poll_ids_poll_entitled_uc7562ed ON nm_meeting_user_entitled_at_poll_ids_poll_entitled_user_t (poll_entitled_user_id);
...
(select array_agg(n.poll_entitled_user_id ORDER BY n.poll_entitled_user_id) from nm_meeting_user_entitled_at_poll_ids_poll_entitled_user_t n where n.meeting_user_id = m.id) as entitled_at_poll_ids,
...
CREATE VIEW "poll_entitled_user" AS SELECT *,
(select array_agg(n.meeting_user_id ORDER BY n.meeting_user_id) from nm_meeting_user_entitled_at_poll_ids_poll_entitled_user_t n where n.poll_entitled_user_id = p.id) as meeting_user_ids
FROM poll_entitled_user_t p;
...
CREATE TRIGGER tr_log_nm_meeting_user_entitled_at_poll_ids_poll_entitlea34b93a AFTER INSERT OR UPDATE OR DELETE ON nm_meeting_user_entitled_at_poll_ids_poll_entitled_user_t
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('meeting_user','meeting_user_id','entitled_at_poll_ids','poll_entitled_user','poll_entitled_user_id','meeting_user_ids');
CREATE CONSTRAINT TRIGGER notify_transaction_end AFTER INSERT OR UPDATE OR DELETE ON nm_meeting_user_entitled_at_poll_ids_poll_entitled_user_t
DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION notify_transaction_end();
...
SQL nt:nt => meeting_user/entitled_at_poll_ids:-> poll_entitled_user/meeting_user_ids
...
SQL nt:1rR => poll/entitled_user_ids:-> poll_entitled_user/poll_id
...
SQL nt:nt => poll_entitled_user/meeting_user_ids:-> meeting_user/entitled_at_poll_ids
...
SQL nt:1Gr => user/poll_option_ids:-> poll_option/content_object_id
