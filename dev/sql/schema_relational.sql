...
-- MODELS_YML_CHECKSUM = '0fc4611ca931b9872d70b2764ee7007c'
...
CREATE TABLE history_entry_t (
    ...
    CONSTRAINT valid_history_entry_model_id_part1 CHECK (split_part(model_id, '/', 1) IN ('assignment','motion','topic','user')),
    position_id integer
...
CREATE TABLE poll_ballot_user_t (
    ...
    acting_user_id integer
        CONSTRAINT required_poll_ballot_user_acting_user_id NOT NULL,
    represented_user_id integer
        CONSTRAINT required_poll_ballot_user_represented_user_id NOT NULL,
...
CREATE TABLE nm_meeting_user_entitled_at_poll_ids_poll_t (
    meeting_user_id integer
        CONSTRAINT required_nm_meeting_user_entitled_at_poll_ids_poll_t_meef9cfab5 NOT NULL
        CONSTRAINT fk_nm_meeting_user_entitled_at_poll_ids_poll_t_meeting_u4b04998 REFERENCES meeting_user_t (id)
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    poll_id integer
        CONSTRAINT required_nm_meeting_user_entitled_at_poll_ids_poll_t_poll_id NOT NULL
        CONSTRAINT fk_nm_meeting_user_entitled_at_poll_ids_poll_t_poll_id_pda65800 REFERENCES poll_t (id)
        ON DELETE CASCADE
        INITIALLY DEFERRED,
    CONSTRAINT pk_nm_meeting_user_entitled_at_poll_ids_poll_t PRIMARY KEY (meeting_user_id, poll_id)
);
CREATE INDEX idx_nm_meeting_user_entitled_at_poll_ids_poll_t_meeting_user_id ON nm_meeting_user_entitled_at_poll_ids_poll_t (meeting_user_id);
CREATE INDEX idx_nm_meeting_user_entitled_at_poll_ids_poll_t_poll_id ON nm_meeting_user_entitled_at_poll_ids_poll_t (poll_id);
...
(select array_agg(n.meeting_user_id ORDER BY n.meeting_user_id) from nm_meeting_user_entitled_at_poll_ids_poll_t n where n.poll_id = p.id) as entitled_meeting_user_ids,
...
(select array_agg(p.id ORDER BY p.id) from poll_ballot_user_t p where p.acting_user_id = u.id) as acting_ballot_ids,
(select array_agg(p.id ORDER BY p.id) from poll_ballot_user_t p where p.represented_user_id = u.id) as represented_ballot_ids,
...
ALTER TABLE history_entry_t ADD CONSTRAINT fk_history_entry_t_model_id_topic_id_topic_t_id FOREIGN KEY(model_id_topic_id) REFERENCES topic_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_history_entry_t_model_id_topic_id ON history_entry_t (model_id_topic_id);
...
ALTER TABLE poll_ballot_user_t ADD CONSTRAINT fk_poll_ballot_user_t_acting_user_id_user_t_id FOREIGN KEY(acting_user_id) REFERENCES user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_ballot_user_t_acting_user_id ON poll_ballot_user_t (acting_user_id);
ALTER TABLE poll_ballot_user_t ADD CONSTRAINT fk_poll_ballot_user_t_represented_user_id_user_t_id FOREIGN KEY(represented_user_id) REFERENCES user_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_ballot_user_t_represented_user_id ON poll_ballot_user_t (represented_user_id);
...
CREATE TRIGGER tr_log_topic_model_id_topic_id AFTER INSERT OR UPDATE OF model_id_topic_id OR DELETE ON history_entry_t
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('topic','model_id_topic_id','history_entry_ids');
...
CREATE TRIGGER tr_log_nm_meeting_user_entitled_at_poll_ids_poll_t AFTER INSERT OR UPDATE OR DELETE ON nm_meeting_user_entitled_at_poll_ids_poll_t
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('meeting_user','meeting_user_id','entitled_at_poll_ids','poll','poll_id','entitled_meeting_user_ids');
CREATE CONSTRAINT TRIGGER notify_transaction_end AFTER INSERT OR UPDATE OR DELETE ON nm_meeting_user_entitled_at_poll_ids_poll_t
DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION notify_transaction_end();
...
CREATE TRIGGER tr_log_poll_ballot_user_t_acting_user_id AFTER INSERT OR UPDATE OF acting_user_id OR DELETE ON poll_ballot_user_t
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('user', 'acting_user_id', 'acting_ballot_ids');
CREATE TRIGGER tr_log_poll_ballot_user_t_represented_user_id AFTER INSERT OR UPDATE OF represented_user_id OR DELETE ON poll_ballot_user_t
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('user', 'represented_user_id', 'represented_ballot_ids');
...
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_meeting_user_t_entitled_at_poll_ids AFTER INSERT ON meeting_user_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals_multi('nm_meeting_user_entitled_at_poll_ids_poll_t', 'meeting_user_id', 'meeting_user', 'poll_id', 'poll', 'meeting_id', 'entitled_at_poll_ids');
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_poll_t_entitled_meeting_user_ids AFTER INSERT ON poll_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals_multi('nm_meeting_user_entitled_at_poll_ids_poll_t', 'poll_id', 'poll', 'meeting_user_id', 'meeting_user', 'meeting_id', 'entitled_meeting_user_ids');
CREATE CONSTRAINT TRIGGER equal_meeting_id_on_meeting_user_t_entitled_at_poll_ids_058a2c8 AFTER INSERT ON nm_meeting_user_entitled_at_poll_ids_poll_t INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION check_equals_intermediate('meeting_user_id', 'meeting_user', 'poll_id', 'poll', 'meeting_id', 'entitled_at_poll_ids');
...
FIELD 1Gr:nt,nt,nt,nt => history_entry/model_id:-> assignment/history_entry_ids,motion/history_entry_ids,topic/history_entry_ids,user/history_entry_ids
...
SQL nt:nt => poll/entitled_meeting_user_ids:-> meeting_user/entitled_at_poll_ids
...
FIELD 1rR:nt => poll_ballot_user/acting_user_id:-> user/acting_ballot_ids
FIELD 1rR:nt => poll_ballot_user/represented_user_id:-> user/represented_ballot_ids
FIELD 1r:nt => poll_ballot_user/acting_meeting_user_id:-> meeting_user/acting_ballot_ids
FIELD 1r:nt => poll_ballot_user/represented_meeting_user_id:-> meeting_user/represented_ballot_ids
...
SQL nt:1Gr => topic/history_entry_ids:-> history_entry/model_id
...
SQL nt:1rR => user/acting_ballot_ids:-> poll_ballot_user/acting_user_id
SQL nt:1rR => user/represented_ballot_ids:-> poll_ballot_user/represented_user_id
