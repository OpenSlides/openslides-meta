...
-- MODELS_YML_CHECKSUM = '7cb0dea6dfac00bae98f59de069b5c03'
...
CREATE TABLE poll_ballot_user_t (
    ...
    poll_ballot_id integer
        CONSTRAINT unique_poll_ballot_user_poll_ballot_id UNIQUE,
...
CREATE VIEW "poll_ballot" AS SELECT *,
(select pb.id from poll_ballot_user_t pb where pb.poll_ballot_id = p.id) as poll_ballot_user_id
FROM poll_ballot_t p;


CREATE VIEW "poll_ballot_user" AS SELECT * FROM poll_ballot_user_t p;
...
ALTER TABLE poll_ballot_user_t ADD CONSTRAINT fk_poll_ballot_user_t_poll_ballot_id_poll_ballot_t_id FOREIGN KEY(poll_ballot_id) REFERENCES poll_ballot_t(id) INITIALLY DEFERRED;
CREATE INDEX idx_poll_ballot_user_t_poll_ballot_id ON poll_ballot_user_t (poll_ballot_id);
...
CREATE TRIGGER tr_log_poll_ballot_user_t_poll_ballot_id AFTER INSERT OR UPDATE OF poll_ballot_id OR DELETE ON poll_ballot_user_t
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('poll_ballot', 'poll_ballot_id', 'poll_ballot_user_id');
...
SQL 1t:1r => poll_ballot/poll_ballot_user_id:-> poll_ballot_user/poll_ballot_id

FIELD 1rR:nr => poll_ballot_user/poll_id:-> poll/ballot_user_ids
FIELD 1r:1t => poll_ballot_user/poll_ballot_id:-> poll_ballot/poll_ballot_user_id
