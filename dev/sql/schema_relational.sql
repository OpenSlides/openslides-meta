...
-- MODELS_YML_CHECKSUM = '6cd61f84a370b5a03a09ff9da2986301'
...
(select array_agg(p.id ORDER BY p.id) from poll_entitled_user_t p where p.meeting_user_id = m.id) as entitled_at_poll_ids,
...
CREATE VIEW "poll_entitled_user" AS SELECT * FROM poll_entitled_user_t p;
...
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('meeting_user', 'meeting_user_id', 'entitled_at_poll_ids');
...
SQL nt:1r => meeting_user/entitled_at_poll_ids:-> poll_entitled_user/meeting_user_id
...
FIELD 1r:nt => poll_entitled_user/meeting_user_id:-> meeting_user/entitled_at_poll_ids
