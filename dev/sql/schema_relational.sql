...
comment on column chat_group_t.name is 'unique in meeting';
...
comment on column motion_t.number is 'unique in meeting';
...
comment on column motion_state_t.name is 'unique for workflow';
...
comment on column option_t.text is 'unique in poll';
comment on column option_t.content_object_id is 'unique in poll';
...
comment on column projector_countdown_t.title is 'unique in meeting';
...
comment on column structure_level_t.name is 'unique in meeting';
...
comment on column structure_level_list_of_speakers_t.structure_level_id is 'unique in list_of_speakers';
...
CREATE TABLE user_t (
    ...
    member_number varchar(256) UNIQUE,
    saml_id varchar(256) UNIQUE CONSTRAINT minlength_saml_id CHECK (char_length(saml_id) >= 1),
    pronoun varchar(32),
    title varchar(256),
